...
public:
	...
	static void DrawText(HDC hDC, CPaintManagerUI* pManager, RECT& rc, 
		LPCWSTR pstrText, DWORD dwTextColor, int iFont, UINT uStyle);
	...
	static SIZE EstimateTextSize(HDC hDC, CPaintManagerUI* pManager, LPCTSTR pstrText, int iFont, UINT uStyle, 
		bool bShowhtml = false, bool bEnabledEffect = false, 
		int nSpecifyWidth = 0, int nSpecifyHeight = 0, const RECT* prcTextpadding = NULL);
};
