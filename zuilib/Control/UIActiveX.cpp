...
STDMETHODIMP CActiveXCtrl::ShowObject(void)
{
	...
	if( m_pViewObject  ) m_pViewObject->Draw(DVASPECT_CONTENT, -1, NULL, NULL, NULL, hDC, (RECTL*) &m_pOwner->m_rcItem, (RECTL*) &m_pOwner->m_rcItem, NULL, NULL);
...
bool CActiveXUI::DoCreateControl()
{
	...
	if( (dwMiscStatus & OLEMISC_INVISIBLEATRUNTIME) == 0 ) {
		Hr = m_pUnk->DoVerb(OLEIVERB_INPLACEACTIVATE, NULL, 
			pOleClientSite, 0, m_pManager->GetPaintWindow(), &m_rcItem);
		/*
		::RedrawWindow(m_pManager->GetPaintWindow(), &m_rcItem, NULL, 
