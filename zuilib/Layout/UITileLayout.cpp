...
namespace zuilib {
	...
	void CTileLayoutUI::SetAttribute(LPCWSTR pstrName, LPCWSTR pstrValue)
	{
		if (_tcscmp(pstrName, _T("itemsize")) == 0) {
			...
			SetFixedColumns(_ttoi(pstrValue));
		else if (_tcscmp(pstrName, _T("rows")) == 0)
			SetFixedRows(_ttoi(pstrValue));
		else if (_tcscmp(pstrName, _T("childvpadding")) == 0)
			SetChildVPadding(_ttoi(pstrValue));
		...
		else	//m_nRowsFixed>0
		{
			...
			if (m_nRows > 1)
			{
				if (iChildVPadding <= 0)
				{
					if (m_pVerticalScrollBar 
						...
						&& rc.bottom - rc.top >= m_nRows * m_szItem.cy)
			...
			for (int it1 = 0; it1 < m_items.GetSize(); it1++)
			{
				...
				if (iChildAlign == DT_CENTER) {
					if (iChildVAlign == DT_VCENTER) {
						RECT rcCtrl = { 
							iPosX + (m_szItem.cx - sz.cx) / 2 + rcPadding.left, 
					...
					if (iChildVAlign == DT_VCENTER) {
						RECT rcCtrl = { 
							iPosX + m_szItem.cx - sz.cx + rcPadding.left, 
							iPosY + (m_szItem.cy - sz.cy) / 2 + rcPadding.top, 
